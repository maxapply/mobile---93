...
>         1. 导入api函数
>         2. click添加事件
>         3. methods方法配合事件，调用api实现添加动作
>             添加评论、回复 判断 分别处理即可
>     
>             其中添加回复稍微繁琐一点
>             繁琐在添加回复完成后，当前评论的回复数量会自动累加
>             怎么累加：
>             把当前评论的记录对象找到，对其中的reply_count做累加
>             this.commentList[下标].reply_count++
>     
>             处于用户体验的角度，给添加动作制作加载效果
...
3. 在`article/components/com-comment.vue`中做如下处理
