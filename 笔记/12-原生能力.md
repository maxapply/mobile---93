...
​	原生能力是  安卓andriod、苹果ios  的事情，但是需要额外学习相关语言，例如安卓的java语言，苹果的object-c语言，针对前段开发人员基本是颠覆性、不现实的，通过如下内容学习，可以知道javascript也可以调用原生能力
...
DCloud面向HTML5行业分别推出了开发工具**HBuilder**、手机强化引擎**5+ Runtime**、跨平台前端框架**mui**、应用发行产品流应用，通过系列产品对HTML5的强化支持，使得HTML5能达到原生的功能和体验，同时在发行上更优于原生应用。 
...
是一款前端框架，类似Vue、react，最接近原生App体验

### uni-app

uni-app 是一个使用 **Vue.js** 开发所有前端应用的框架，开发者编写**一套**代码，可发布到iOS、Android、H5、以及各种小程序（微信/支付宝/百度/头条/QQ/钉钉）等多个平台
...
公众服务：天气、新闻、体育、娱乐信息获取

个人信息服务：收发电子邮件

商业应用：股票交易、银行业务、网上购物、机票及酒店预订等
...
3. 在页面的主内容容器中输入一个九宫格 **`mGrid`**, 然后 把图标换成  拍照 /  电话 

![1583570304606](img(online)/1583570304606.png)
...
现在有可能大家还不知道项目的原生能力在哪，通过hbuilderx创建的项目，里边哟一个名称为  `manifest.json` 的文件 [说明地址](https://ask.dcloud.net.cn/article/94) ，该文件是一个w3c的webapp配置文件，即表明拥有原生能力
...
> 因为开发的是移动端app项目，要体现原生能力，所以需要在手机上运行
>
> 我们开发的是andriod的app项目(苹果标准比较多，开发费事)，如果没有andriod手机，可以在电脑上安装 **模拟器** 来运行
...
![1583571806424](img(online)/1583571806424.png)
...
1. 给拍照的图标设置`id=“photo”`属性，使得可以被找到，和显示图片的img标签
   ...
   <img src="" id="mypic" style="display:block;width:300px; height:200px;margin:20px;" >
   ```

   

2. 在页面**底部**给上述li标签设置单击，以调用摄像头
   ...
   <script type="text/javascript">
     document.querySelector('#photo').addEventListener('tap',()=>{
       ...
       cmr.captureImage(function(url){
         // 拍照成功之后执行，返回图片路径 
         // 但是这个路径并不能直接使用，其实相对路径，需要转换为绝对路径
         const absurl = plus.io.convertLocalFileSystemURL(url)
         // 显示在页面上
         document.querySelector('#mypic').src = absurl
       })	
     })
   </script>
   ```
...
​	如果拍照有问题，可以把 **模拟器** 和 **hbuilderX**  重新启动再连接调试
...
![1583574496608](img(online)/1583574496608.png)
...
2. 在拍照代码的后边，打电话的代码
   ...
   ```js
   // 打电话
   document.querySelector('#phone').addEventListener('tap',()=>{
     // plus.device.dial(手机号码, 询问true/直拨false);
