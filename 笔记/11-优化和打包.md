...
在`layout/index.vue`组件中 给 子路由 占位符标签通过 keep-alive包裹，以达到缓存效果
...
1. 下载名称为`HBuilderX`的编辑器软件
   ...
   ![1584361670281](img(online)/1584361670281.png)
...
​	如果通过微信上传app到手机，有可能文件名称默认被额外添加**.1**的后缀名标志，导致软件**不能**安装，请注意修改去除
