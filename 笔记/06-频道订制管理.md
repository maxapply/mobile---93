...
1. 创建api方法，api/channel.js
   ...
   export async function apiChannelAdd (channel) {
     ...
     const cacheChannels = JSON.parse(localStorage.getItem(key)) // 获取缓存
     cacheChannels.push(channel) // 添加
     // 重新写入缓存
...
1. 我的频道 更新添加
2. 频道推荐 会减少对应的频道(因为有computed响应式)
 ...
 * @param {被删除频道的id} id
 */
export async function apiChannelDel (id) {
    const key = store.state.user.token ? CHANNEL_KET_VIP : CHANNEL_KEY_TRAVEL // 获取缓存的key
    ...
    const tmpChannels = cacheChannels.filter(item => item.id !== id)
